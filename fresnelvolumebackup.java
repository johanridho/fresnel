...
//        System.out.println("aaaaaaa"+jmlSrcRcv);
        ...
        LinkedList<Point> SR = new LinkedList<Point>();
        int iterasiFMK=0;
...
//                    System.out.println("bbbb = "+b);                        
        ...
        for (int i=0; i<SR.size(); i+=2) {
            Point P = SR.get(i);
            Point Q = SR.get(i+1);
            System.out.println(P.getX()+" - "+P.getY()+" -- "+i);     
//            makeEllipse(i, arraySrcRcv[(int)P.getX()], arraySrcRcv[(int)P.getY()]);
            final int jjj = i;
            final Point ppp = P;
            final Point qqq = Q;
            ...
            T.add(new Thread(new Runnable() {
                    public void run() {
                        makeEllipse (jjj, arraySrcRcv[(int)ppp.getX()], arraySrcRcv[(int)ppp.getY()]);
                        makeEllipse (jjj, arraySrcRcv[(int)ppp.getX()], arraySrcRcv[(int)ppp.getY()]);
                    }
                })
            );
...
//        t2.start();
//        
//        while(t0.isAlive() || t1.isAlive() || t2.isAlive()){          //loop utk memastikan thread sudah mati smua
//            
//        }                
...
//        t5.start();
//        
//        while(t0.isAlive() || t1.isAlive() || t2.isAlive() || t3.isAlive() || t4.isAlive() || t5.isAlive()){          //loop utk memastikan thread sudah mati smua
//            
//        }                
